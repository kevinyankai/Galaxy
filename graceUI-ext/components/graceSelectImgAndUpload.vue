<template>
	<view class="grace-add-list">
		<view class="grace-add-list-items" v-for="(item, index) in imgLists" :key="index">
			<image :src="siteUrl+item.url" :data-imgurl="item.url" @tap="showImgs(index)" class="grace-add-list-img" :mode="imgMode"></image>
...
export default {
    ...
    methods:{
		...
		currentImg: function(){
			this.$emit('currentImg', this.imgLists);
		},
        ...
        showImgs : function(index){
        	...
        	for(let i = 0; i < this.imgLists.length; i++){
        		imgs.push(this.$store.state.siteUrl+this.imgLists[i].url);
        	}
            uni.previewImage({
              urls: imgs,
              current : index
...
.grace-add-list-img{width:222rpx;height: inherit !important}
</style>