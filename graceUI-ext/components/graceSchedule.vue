		...
		<view class="grace-date-days">
			<block v-for="(item, index) in days" :key="index">
				...
				:style="{background:currentDayIn == cYear+'-'+cMonthStr+'-'+ item.date ? activeBgColor : bgColor}">
					...
					<text v-if="item.sign==='yes'" class="grace-icons" style="color: #007AFF; font-size: 18rpx;">&#xe7f8;</text>
...
export default{
  ...
  onShow(){
	...
	methods:{
    getUserSign() {
      this.$qyc.request("/f/work_businesscard/businesscardSign/getUserSign", {}).then(res => {
        if (res.data.result === "true") {
          this.listDay = res.data.list;
          for(let i =1;i<this.days.length;i++){
            for(let j = 0;j<this.listDay.length;j++){
              if(this.listDay[j] === this.days[i].date ){
                this.days[i].sign='yes';
              }
            }
          }
        }
