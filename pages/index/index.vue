<template>
	<view class="content">
		...
		<view class="header1" v-if="top==1">
			<image src="../../static/images/logow.png"></image>
			<view class="pro-title1">galaxy</view>
			<view class="pro-seach1">
		...
		<view class="header" v-else>
			...
			<view class="header-con">
				<view class="pro-name">Galaxy</view>
				<view class="pro-des">
					<view>Various data and creations on the chain. </view>
					<view>Get support from creators around the world.</view>
				</view>
				<view class="pro-seachbox">
					<view class="pro-seach" :class="showpopbox==true?'pro-seachnose':'pro-seachisse'">
						<input placeholder="Search cid or data set" value="" v-model="keywords" @click.stop="Listeningfocus"
						 placeholder-style="color:#CCCCCC" />
						<image src="../../static/images/search.png" @click="search"></image>
					</view>
					<view class="pop-box" v-show="showpopbox==true">
						<view class="search-box">
							<view class="search-boxtitle">Recent search</view>
							<view class="search-cellbox" @click="cidgotodetail(item)" v-for="(item,index) in searchData" :key="index">{{item}}</view>
						</view>
						<view class="dataset-box">
							<view class="dataset-boxtitle">Data set</view>
							<view class="dataset-cellbox">
								<view @click="setdatasearch(item)" v-for="(item,index1) in keyworddata" :key="index1">
									<a :title="item.description">{{item.name}}</a>
								</view>
							</view>
						</view>
					</view>
				</view>
				<view class="pro-cont">Trends: pictures, videos, audio...</view>
			</view>
		</view>
		<view :class="top==1?'tabar-box':'tabar-boxnew'">
			<view class="tabar-title">
				<view :class="{tabbarchecked:index==nowIndex}" @click="tabbarchecked(index)" v-for="(item,index) in tabartitle"
				 :key="index">{{item}}</view>
			</view>
		</view>
		<view v-if="nowIndex==0">
			<view class="content-cellimgbox">
				<view class="content-cellimg">
					<!--<view class="content-cellimg" @click="gotoimgdetail(item)" v-for="(item,index) in celllist">
					 <image class="music-img" :src="item.path" mode="widthFix"></image> -->
					<view hidden>
						<block v-for="(item,index) in imgList" :key="index">
							<image @load="imageLoad" :data-src="item.path" :data-payloadCid="item.payloadCid" :src="item.path" mode="widthFix"></image>
						</block>
					</view>
					<view>
						<block v-for="(item,index) in imgList1" :key="index" v-if="imgShow">
							<image :src="item.src" @click="gotoimgdetail(item)" mode="widthFix" :style="`left: ${item.left}rpx;top: ${item.top}rpx;`"></image>
						</block>
					</view>
				</view>
		...
		<view v-if="nowIndex==1">
			<view class="content-cell">
				<view @click="gotomusicdetail(item)" class="content-cellmusic" v-for="(item,index) in celllist">
					<image class="music-img" src="../../static/images/musics.png" mode="widthFix"></image>
					<a :title="item.payloadCid">
						<view>{{item.payloadCid}}</view>
					</a>
				</view>
			</view>
		</view>
		<view v-if="nowIndex==2">
			<view class="content-cell ">
				<view @click="gotovideodetail(item)" class="content-cellvideo" v-for="(item,index) in celllist">
					<video class="video-box" :src="item.path" loop="false"></video>
					<a :title="item.payloadCid">
						<view>{{item.payloadCid}}</view>
					</a>
				</view>
...
<script>
	...
	var imgWidth = 320 //单张图片的宽度，可通过调整宽度显示不同列数
	colCount = parseInt(1500 / imgWidth) //计算出列数，这里是两列
	...
	export default {
		data() {
			return {
				nowIndex: 0,
				tabartitle: ['Pictures', 'Audio', 'Videos'],
				slidervalue: 0,
				celllist: [],
				keywords: '', //关键字
				keywordsvalue: '',
		...
		methods: {
			// onReachBottom() {
			// 	this.firstrequest((type) => {
			// 		this.loadText = '上拉加载更多...'
			// 	})
			// 	var imgList = []
			// 	this.imgList.push(imgList)
			// },
			//进入页面请求(1,video 2,music,3,picture 4,document 5,other)
			imageLoad(e) {
				_this=this
				
				var src = e.currentTarget.dataset.src //图片地址
				var payloadCid = e.currentTarget.dataset.payloadcid
			...
			firstrequest(type) {
				...
				_this.$qyc.request("/f/api/work_linden/listAll", data).then(function(res) {
					if (res.data.result === "true") {
						// let data = res.data.page.list
						// _this.celllist = [..._this.celllist, ...data]
						_this.celllist = res.data.page.list
						_this.imgList = _this.celllist
					} else {
						uni.showToast({
							icon: 'none',
			...
			searchsetdata() {
				_this = this;
				let data = {};
				_this.$qyc.request("/f/api/work_linden/listDataSetInfo", data).then(function(res) {
					if (res.data.result === "true") {
						_this.keyworddata = res.data.list
			...
			searchkeyword(keywords) {
				...
				_this.$qyc.request("/f/api/work_linden/listByDataSetName", data).then(function(res) {
					if (res.data.result === "true") {
						_this.celllist = res.data.listSource.list
						_this.imgList = _this.imgList.concat(_this.celllist)
						_this.imgShow=true
					} else {
						uni.showToast({
							icon: 'none',
				...
				if (this.keywords == '') {
					...
					if (this.keywords.length > 50) {
						if (this.searchData.length < 5) {
							this.searchData.push(this.keywords)
							uni.setStorage({
								key: "searchData",
								data: JSON.stringify(this.searchData)
							})
						} else {
							this.searchData.splice(4, 1)
				...
				_this.$qyc.request("/f/api/work_linden/getDetailsByCid", data).then(function(res) {
					if (res.data.result === "true") {
						if (res.data.details.types == 1) {
							uni.navigateTo({
								url: './videodetail?cid=' + cid
							})
						} else if (res.data.details.types == 2) {
							uni.navigateTo({
								url: './musicdetail?cid=' + cid
							})
						} else if (res.data.details.types == 3) {
							uni.navigateTo({
								url: './imagedetail?cid=' + cid
							})
						}
					} else {
						uni.showToast({
			...
			gotoimgdetail(item) {
				uni.navigateTo({
					url: './imagedetail?cid=' + item.payloadCid
				})
			},
			//音乐
			gotomusicdetail(item) {
				uni.navigateTo({
					url: './musicdetail?cid=' + item.payloadCid
				})
			},
			//视频
			gotovideodetail(item) {
				uni.navigateTo({
					url: './videodetail?cid=' + item.payloadCid
				})
			},
			onPageScroll(res) {
...
<style lang="scss" scoped>
	...
	.dataset-cellbox {
		...
		view {
			...
			display: inline-block;
			background: #FFFFFF;
			box-shadow: 0rpx 8rpx 24rpx 0rpx rgba(144, 149, 172, 0.15);
			border-radius: 12rpx;
		}
